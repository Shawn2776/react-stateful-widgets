...
export default function Programmers() {
  ...
  const [ devNames, setDevNames ] = useState(listOfAwesome);
  const [ devIDs, setDevIDs ] = useState(listOfAwesome);
  ...
  const style = {
    ...
    color: devIDs === null ? 'royalblue' : 'crimson'
  };
      ...
      <div className='programmers'>
        {
          ...
          devNames.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { devIDs }}>Feature</button>
            </div>
          )
        }
      </div>
