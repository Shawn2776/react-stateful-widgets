...
export default function Programmers() {
  const [ devName, setDevName ] = useState(listOfAwesome)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: devID !== null ? 'gold' : 'royalblue'
  };
      ...
      <div className='programmers'>
        {
          ...
          devName.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setDevID(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
